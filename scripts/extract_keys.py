...
KEYS_FILE = os.path.expanduser(os.environ.get("MACBOOK_WECHAT_ZHUAQU_KEYS_FILE", "~/.config/wechat-keys.json"))
CONFIG_FILE = os.path.expanduser(os.environ.get("MACBOOK_WECHAT_ZHUAQU_CONFIG_FILE", "~/.config/wechat-daily.json"))
...
if (!CCKeyDerivationPBKDF) {
    ...
    Interceptor.attach(CCKeyDerivationPBKDF, {
        ...
        onLeave: function(retval) {
            try {
                if (this.passwordLen <= 0 || this.passwordLen > 256 || this.saltLen < 0 || this.saltLen > 32) {
                    return;
                }
                if (!isReadablePointer(this.password) || !isReadablePointer(this.derivedKey)) {
                    return;
                }
                if (this.saltLen > 0 && !isReadablePointer(this.salt)) {
                    return;
                }
                var saltHex = '';
                if (this.saltLen > 0) {
                    saltHex = buf2hex(this.salt.readByteArray(this.saltLen));
                }
                var dkHex = buf2hex(this.derivedKey.readByteArray(this.derivedKeyLen));
                var pwHex = buf2hex(this.password.readByteArray(this.passwordLen));
                ...
                send({type: 'error', msg: e.toString() + (e.stack ? '\n' + e.stack : '')});
            }
        }
    });
}
...
def extract_keys():
    ...
    host_script = (
        FRIDA_HOST
        .replace("__WECHAT_PATH__", wechat_binary)
        .replace("__JS_CODE__", FRIDA_JS.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n"))
    )
...
def detect_databases():
    ...
    for db_name, db_path in db_files.items():
        ...
        # Match by SQLCipher page-0 salt. The captured PBKDF2 outputs are 32-byte AES keys.
        matched = False
